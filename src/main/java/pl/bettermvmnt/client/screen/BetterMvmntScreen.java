...
public final class BetterMvmntScreen extends Screen {
    ...
    private BetterMvmntScreen(Tab tab) {
    ...
    protected void init() {
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.literal("HUD"), btn -> this.client.setScreen(new BetterMvmntScreen(Tab.HUD)))
                .dimensions(centerX - 5, baseY, 60, 20).build());
        this.addDrawableChild(ButtonWidget.builder(Text.literal("ESP"), btn -> this.client.setScreen(new BetterMvmntScreen(Tab.ESP)))
                .dimensions(centerX + 60, baseY, 60, 20).build());
        this.addDrawableChild(ButtonWidget.builder(Text.literal("ANIM"), btn -> this.client.setScreen(new BetterMvmntScreen(Tab.ANIMATION)))
                .dimensions(centerX + 125, baseY, 70, 20).build());
        this.addDrawableChild(ButtonWidget.builder(Text.literal("EXTRA"), btn -> this.client.setScreen(new BetterMvmntScreen(Tab.EXTRA)))
    ...
    private void initEspTab(int centerX, int y) {
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.literal("Change ESP Color"), btn -> {
            int index = colorIndex(ESP_COLORS, BetterMvmntClient.SETTINGS.espColor);
            BetterMvmntClient.SETTINGS.espColor = ESP_COLORS[(index + 1) % ESP_COLORS.length];
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.literal("Hitbox +"), btn ->
                BetterMvmntClient.SETTINGS.hitboxScale = Math.min(2.50f, BetterMvmntClient.SETTINGS.hitboxScale + 0.05f))
                .dimensions(centerX + 5, y + 96, 115, 20).build());

        this.addDrawableChild(ButtonWidget.builder(Text.literal("Chinese Hat: " + onOff(BetterMvmntClient.SETTINGS.chineseHatEnabled)), btn -> {
            BetterMvmntClient.SETTINGS.chineseHatEnabled = !BetterMvmntClient.SETTINGS.chineseHatEnabled;
            btn.setMessage(Text.literal("Chinese Hat: " + onOff(BetterMvmntClient.SETTINGS.chineseHatEnabled)));
        }).dimensions(centerX - 120, y + 120, 240, 20).build());
